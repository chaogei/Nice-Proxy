...
| NFR-9 | 安全 | 认证凭据加密存储；Clash API 仅监听 127.0.0.1 且带随机 secret；默认不开放无认证的 `0.0.0.0` 监听（需二次确认） |
...
| 开启 `0.0.0.0` 且无认证 | 弹出风险确认："同一网络下的任何设备都能使用你的代理"，需显式确认 |
...
| 风险 | 缓解措施 |
| --- | --- |
| 局域网内被他人蹭用代理 | 默认建议开启认证；`0.0.0.0` + 免认证组合需二次确认；首页醒目展示"未认证"警告徽章 |
